...
def train_nerf(config):
    """
    Trains a NeRF model using the provided configuration.

    :param config: A configuration object containing the necessary hyperparameters.
    """
    ...
    for epoch in range(config.num_epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{config.num_epochs}, Loss: {epoch_loss / len(dataloader)}"
        )
